...
export class Game {
  ...
  move() {
    ...
    let deadCellFrequency = new Collections.Dictionary((cell: ICell) =>
      cell.getId()
    );
    this.world.getLiveCells().forEach(liveCell => {
      ...
      this.world.getDeadNeighbors(liveCell).forEach(deadCell => {
        if (deadCellFrequency.containsKey(deadCell))
          deadCellFrequency.setValue(
            deadCell,
            <number>deadCellFrequency.getValue(deadCell) + 1
          );
        else deadCellFrequency.setValue(deadCell, 1);
      });
    });
    deadCellFrequency.forEach((cell, count) => {
      if (count === 3) newWorld.addLiveCell(cell);
    });
    this.world = newWorld;
  }
