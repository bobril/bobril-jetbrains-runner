...
import { ICell } from "../src/model/cell2";
...
describe("Game", () => {
  ...
  describe("getLiveCells", () => {
    ...
    it("getLiveCells will return values from nextIteratinorld adfer move", () => {
      ...
      game.move();
      expect(game.getLiveCells()).toEqual([
        nextIterationLiveCell,
        nextIterationLiveCell
      ]);
    });
  ...
  describe("Rules", () => {
    describe("Every life cell with les then two neighbors will die", () => {
      it("Cell with zero live neighbor will not be added to next iteration", () => {
        currentWorld.getLiveNeighbors.and.returnValue([]);
        game.move();
        expect(nextIterationWorld.addLiveCell).not.toHaveBeenCalledWith(
          currentCell
        );
      });
    ...
    describe("Every life cell with two or three neighbors will survive", () => {
      it("Cell with two neighbors will be added to nextIterationWorld", () => {
        ...
        game.move();
        expect(nextIterationWorld.addLiveCell).toHaveBeenCalledWith(
          currentCell
        );
      });
